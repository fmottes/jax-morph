...
def CellTypeImbalance(metric="number"):
    ...
    if "number" == metric:
        ...
        def _cost(trajectory):
            ...
            cost = trajectory.celltype.sum(-2) @ np.asarray([1.0, -1.0])
    ...
    # CELL TYPE DISTRIBUTION ENTROPY
        ...
        def _cost(trajectory):
            ...
            p = trajectory.celltype.sum(-2) / trajectory.celltype.sum(-2).sum(
                -1, keepdims=True
            )
...
### ELONGATION
def MeanSquareY(*, nonsymm_penalty=1.0, realign=False):
...
### V SHAPE
def VShape(*, cost_per_cell=1, rew_per_cell=3, nonsymm_penalty=0.1, realign=False):
    ...
    def _cost(trajectory):
        ...
        mask = (
            (pos[:, :, 1] + 1.5 > 0.5 * np.abs(pos[:, :, 0]))
            * (pos[:, :, 1] + 1.5 < 3.5 + 0.5 * np.abs(pos[:, :, 0]))
            * (pos[:, :, 1] > -0.1)
        )

        cost = np.sum(
            np.where(mask, -rew_per_cell, cost_per_cell) * trajectory.celltype.sum(-1),
            axis=-1,
        )
...
def CVDivrates():
    def _cost(trajectory):
        return np.std(trajectory.division[:, :], axis=1) / np.mean(
            trajectory.division[:, :], axis=1
        )

    return _cost


def PairwiseLoss(coeffs=None, kT_reward=0.01, overlap_penalty=1.0):
    def _cost(trajectory):

        displacement_fn = jax.vmap(
            jax.vmap(trajectory.displacement, (0, None)), (None, 0)
        )
        displacements = (
            jax.vmap(displacement_fn, (0, 0))(trajectory.position, trajectory.position)
            + 1e-12
        )
        ...
        type_pair = jax.vmap(jax.vmap(type_pair_f, (-1, None)), (None, -1))(
            trajectory.celltype, trajectory.celltype
        )

        distance_pair = jax.vmap(lambda x: np.where(x, distances, 0.0), 0)(type_pair)

        costs = np.sum(distance_pair, (-2, -1))
        overlap = np.sum(np.where(distances < 0.85, 1.0, 0.0), (-2, -1))

        final_costs = (
            np.sum(coeffs[:, :, None] * costs, (0, 1))
            - kT_reward * trajectory.kT.sum()
...
def MeanSquareZ(*, nonsymm_penalty=1.0, realign=False, negative=True):

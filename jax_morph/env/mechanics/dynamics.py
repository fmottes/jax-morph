...
class SGDMechanicalRelaxation(SimulationStep):
    ...
    def _sgd(self, state, pair_potential):
        ...
        init, apply = jax_md.minimize.gradient_descent(
            pair_potential, state.shift, self.dt
        )

        def scan_fn(opt_state, i):
            return apply(opt_state), 0.0

        # relax system
        opt_state = init(state.position)
        opt_state, _ = jax.lax.scan(
            scan_fn, opt_state, np.arange(self.relaxation_steps)
        )
...
class BrownianMechanicalRelaxation(SimulationStep):
    ...
    dt: float = eqx.field(default=8e-4, static=True)
    kT: float = eqx.field(default=1.0, static=True)
    gamma: float = eqx.field(default=0.8, static=True)
    discount: float = eqx.field(default=1.0, static=True)
    ...
    def _brownian(self, state, energy_fn, kT, gamma, key):
        ...
        init, apply = jax_md.simulate.brownian(
            energy_fn, state.shift, dt=self.dt, kT=kT, gamma=gamma
        )
        ...
        def scan_fn(opt_state, i):
            ...
            opt_state = eqx.tree_at(
                lambda s: s.position,
                opt_state,
                replace_fn=lambda p: discount_tangent(p, self.discount),
            )
            return opt_state, 0.0

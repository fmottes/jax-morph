...
class LocalChemicalGradients(SimulationStep):
    ...
    def __call__(self, state, *, key=None, **kwargs):
        ...
        safe_dist = np.where(dist > 0.0, dist, 1)
        dist = np.where(dist > 0.0, safe_dist, 0)
        ...
        norm_disp = np.where(
            (dist > 0)[:, :, None], (disp * neig[:, :, None]) / safe_dist[:, :, None], 0
        )
...
class LocalMechanicalStress(SimulationStep):
    ...
    def __call__(self, state, *, key=None, **kwargs):
        ...
        drs = jax_md.space.map_product(state.displacement)(
            state.position, state.position
        )

        stresses = (
            np.sum(np.multiply(forces, np.sign(drs)), axis=(0, 2))[:, None] / 1e2
        )  # heuristic rescaling for compatibility with other cell inputs
        stresses = np.where(state.celltype.sum(1)[:, None] > 0, stresses, 0.0)

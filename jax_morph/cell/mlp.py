...
class DivisionMLP(SimulationStep):
    ...
    def __init__(
        ...
        *,
        key,
        w_init=jax.nn.initializers.constant(0.0),
        ...
        **kwargs
        ...
        self.mlp = eqx.tree_at(
            ...
            self.mlp,
            w_init(key, self.mlp.layers[0].weight.shape),
        )
    ...
    def __call__(self, state, *, key=None, **kwargs):
        ...
        division = np.where(state.celltype.sum(1)[:, None] > 0, division, 0.0)
...
class SecretionMLP(SimulationStep):
    ...
    def __init__(
        ...
        *,
        ...
        transform_output=None,
        final_activation=lambda x: jax.nn.sigmoid(x / 10.0),
        **kwargs
    ):
...
class HiddenStateMLP(SimulationStep):
    ...
    def __call__(self, state, *, key=None, **kwargs):
        ...
        # keep only alive cells
        delta_hs = np.where(state.celltype.sum(1)[:, None] > 0, delta_hs, 0.0)

        hidden_state = (
            self.memory_decay * state.hidden_state + (1 - self.memory_decay) * delta_hs
        )
...
class CellStateMLP(SimulationStep):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.out_indices = tuple(
            (out_shape - np.cumsum(np.asarray(out_sizes)[::-1])).tolist()[::-1]
            + [out_shape]
        )
        ...
        if w_init is not None:
            ...
            substitute = lambda x, k: (
                w_init(k, x.shape) if (isinstance(x, jax.Array) and x.ndim > 1) else x
            )

            leaves, treedef = jax.tree_flatten(self.mlp)

            self.mlp = jtu.tree_unflatten(
                treedef,
                [
                    substitute(leaf, k)
    ...
    def __call__(self, state, *, key=None, **kwargs):
        ...
        for i, field in enumerate(self.output_fields):
            ...
            new_field = (
                self.memory * getattr(state, field) + (1 - self.memory) * new_field
            )
